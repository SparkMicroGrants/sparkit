...
class SparkProject(models.Model):
	...
	total = fields.Float(string="Total Budget", readonly=True,
		...
		help="Includes both Spark, community, FVS-AMADE, and other contributions.",
		track_visibility='onchange')
	...
	def _fvs_contribution(self):
		for r in self:
			r.fvs_contribution = sum(s.budgeted for s in r.budget_line_item_ids if s.source == "fvs")
	...
	#Counts the percentage of the total project budget contributed by FVS
...
class ProjectBudgetItem(models.Model):
	...
	source = fields.Selection([('spark', 'Spark'),
		 ...
		 ('fvs', 'FVS-AMADE'), ('other', 'Other')], select=True,
